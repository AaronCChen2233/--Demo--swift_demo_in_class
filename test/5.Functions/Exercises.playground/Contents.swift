...
func removefristgetlast(s:String)->String{
    var s1 = s
    for i in 0..<s1.count-1{
         s1.remove(at: String.Index(encodedOffset: 0))
    }
   
    return s1
}

print(removefristgetlast(s:word))
//: __Problem 2__
//:
...
func question2(inputarra: Array<String>)->String{
    var lastletterArray = inputarra.map{$0.suffix(1)}
    return lastletterArray.joined(separator: "")
}
print(question2(inputarra: nonsenseArray))
...
func checkIsnumber(price:String)->Bool{
    return Int(price) != nil
}

print(checkIsnumber(price: "test"))
print(checkIsnumber(price: "123"))
...
print(clean(arr: dirtyWordsArray))
